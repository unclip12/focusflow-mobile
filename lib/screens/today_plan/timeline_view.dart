  ...
  List<_TimelineItem> _buildTimelineItems(_TimelineBounds bounds) {
    ...
    if (liveActivity != null) {
      final startedAt = liveActivity.startedAt != null
          ...
          : null;
      liveStartMinutes = startedAt != null
          ? startedAt.hour * 60 + startedAt.minute
          : _currentMinutesOfDay;
      liveEndMinutes = math.max(liveStartMinutes + 1, _currentMinutesOfDay);
    }
    bool liveActivityProcessed = liveActivity == null;
    ...
    while (sliceIndex < sortedSlices.length ||
      ...
      final nextLiveStart =
          !liveActivityProcessed ? liveStartMinutes! : bounds.endMinutes + 1;

      final minStart = math.min(
        nextBlockStart,
        math.min(nextReminderStart, nextLiveStart),
      );

      if (minStart == bounds.endMinutes + 1) {
        break;
      }

      if (minStart == nextReminderStart && nextReminder != null) {
        ...
        reminderIndex++;
      } else if (minStart == nextLiveStart && liveActivity != null) {
        if (liveStartMinutes! > cursor && (liveStartMinutes - cursor) >= 5) {
          items.add(
            _TimelineItem.gap(
              gapStartMinutes: cursor,
              gapEndMinutes: liveStartMinutes,
            ),
          );
        }

        items.add(
          _TimelineItem.liveActivity(liveActivity),
        );
        cursor = math.max(cursor, liveEndMinutes!);
        liveActivityProcessed = true;
  ...
  Widget build(BuildContext context) {
    ...
    return ReorderableListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (item.isGap) {
          final gapKey = 'gap_${item.gapStartMinutes}_${item.gapEndMinutes}';
          final isExpanded = _expandedGaps.contains(gapKey);
          final gapMinutes = item.gapEndMinutes! - item.gapStartMinutes!;
          ...
          return KeyedSubtree(
            ...
            child: _GapSlot(
              ...
              onToggleExpand: () {
                setState(() {
                  if (_expandedGaps.contains(gapKey)) {
                    _expandedGaps.remove(gapKey);
                  } else {
                    _expandedGaps.add(gapKey);
                  }
                });
              },
              gapHeight: gapHeight,
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            _to12h(_minutesToHHMM(DateTime.now().hour * 60 + DateTime.now().minute)),
                            textAlign: TextAlign.right,
                            style: TextStyle(
                              fontSize: 11,
                              fontWeight: FontWeight.w700,
              ...
              SizedBox(
                ...
                child: Stack(
                  ...
                  children: [
                    ...
                    // Solid start dot at top
                    Positioned(
                      top: 6,
                      left: (_kTimelinePillWidth - 10.0) / 2,
                      child: Container(
                        width: 10,
                        height: 10,
                        decoration: BoxDecoration(
...
class _GapSlot extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double outerVerticalPadding = isExpanded ? 8.0 : 4.0;
    final double innerVerticalPadding = isExpanded ? 10.0 : 6.0;
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: DecoratedBox(
                          ...
                          child: Padding(
                            padding: EdgeInsets.fromLTRB(14, innerVerticalPadding, 14, innerVerticalPadding),
                            child: Row(
                              children: [
                                Expanded(
                                  child: hasPastSection && hasFutureSection
                                      ? Column(
                                          crossAxisAlignment: CrossAxisAlignment.start,
                                          mainAxisAlignment: MainAxisAlignment.center,
                                          children: [
                                      ...
                                      : Column(
                                          ...
                                          children: [
                                            ...
                                            else if (hasFutureSection && onAddTask != null)
                                              _GapActionButton(
                                                onPressed: onAddTask,
                                                icon: const Icon(
                                                  Icons.add_rounded,
