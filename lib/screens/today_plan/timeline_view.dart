...
class _TimelineViewState extends State<TimelineView> {
  // Sorted non-locked blocks are exposed for drag reordering.
  // Locked blocks (events, prayers) stay pinned.
  List<Block> _sortedBlocks = [];
  ...
  void _updateSorted() {
    _sortedBlocks = List<Block>.from(widget.blocks)
      ..sort((a, b) =>
  ...
  void _onBlockLongPress(Block block) {
    if (_isLocked(block)) return; // locked blocks can't be dragged via long-press
    HapticsService.medium();
    _showEditSheet(block);
  }

  // ── Read-only detail sheet for locked (event/prayer) blocks ──
  void _showLockedDetailSheet(Block block) {
    ...
    final dur = _toMinutes(block.plannedEndTime) - _toMinutes(block.plannedStartTime);
        ...
        child: Column(
          ...
          children: [
            ...
            _detailRow(Icons.access_time_rounded,
                '$startLabel – $endLabel', cs),
            const SizedBox(height: 8),
            _detailRow(Icons.timer_outlined, _formatDuration(dur > 0 ? dur : block.plannedDurationMinutes), cs),
            const SizedBox(height: 8),
            _detailRow(Icons.push_pin_rounded,
                block.isEvent ? 'Fixed Event — scheduler won\'t move this'
                    : 'Prayer time — auto-inserted', cs),
  ...
  Widget _detailRow(IconData icon, String text, ColorScheme cs) {
    return Row(children: [
  ...
  Widget build(BuildContext context) {
    ...
    // Separate draggable blocks from gaps (gaps are not draggable)
    // We use a simple ListView with manual drag handles for non-locked blocks.
    // Locked blocks and gaps are rendered inline (non-interactive for drag).
    return ReorderableListView.builder(
      padding: EdgeInsets.fromLTRB(0, 8, 0, MediaQuery.of(context).padding.bottom + 72 + 24),
      onReorder: (oldIdx, newIdx) {
        // Map list indices back to block indices (skip gaps)
        final blockItems = items.where((i) => !i.isGap).toList();
        final oldBlock = oldIdx < blockItems.length ? blockItems[oldIdx].block : null;
        final newBlock = newIdx < blockItems.length ? blockItems[newIdx].block : null;
        if (oldBlock == null) return;
        // Find positions in draggable-only list
        final draggable = _sortedBlocks.where((b) => !_isLocked(b)).toList();
        final oIdx = draggable.indexWhere((b) => b.id == oldBlock.id);
        if (oIdx < 0 || _isLocked(oldBlock)) return;
        int nIdx = newBlock != null ? draggable.indexWhere((b) => b.id == newBlock.id) : draggable.length;
        if (nIdx < 0) nIdx = draggable.length;
        _onReorder(oIdx, nIdx);
      },
      ...
      itemBuilder: (context, index) {
        ...
        if (item.isGap) {
          // Gaps get a unique key but are not draggable
          return KeyedSubtree(
...
class _BlockCard extends StatelessWidget {
  ...
  const _BlockCard({required this.block, required this.isDark, this.onLongPress, this.onTap});
